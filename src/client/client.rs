...
use crate::client::connect::http::HttpConnect;
use crate::client::connect::pool::{self, Ver};
use crate::client::connect::{Alpn, Connect, Connected, Connection};
...
pub struct Client<C, S> {
    ...
    pool: pool::Pool<PoolClient<S>, PoolKey>,
}
    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    async fn try_send_request(
        ...
        if let Some(on_conn) = req
            .extensions_mut()
            .get_mut::<crate::client::connect::CaptureConnection>()
        {
            on_conn.call(&pooled.conn_info);
        }
    ...
    ) -> impl Lazy<Output = Result<pool::Pooled<PoolClient<B>, PoolKey>, Error>> + Send + Unpin
    {
        ...
        hyper_lazy(move || {
            ...
            Either::Left(
                connector
                    .connect(crate::client::connect::sealed::Internal, dst)
...
pub struct Builder {
    ...
    #[cfg(feature = "http2")]
    h2_builder: hyper::client::conn::http2::Builder<Exec>,
    pub(crate) pool_config: pool::Config,
    pub(crate) pool_timer: Option<timer::Timer>,
}
...
impl Builder {
    ...
    ///
    /// Return a mutable reference to the inner HTTP/2 builder.
    #[cfg(feature = "http2")]
    #[cfg_attr(docsrs, doc(cfg(feature = "http2")))]
    ...
    pub fn build_http<H, B>(&self, mut connector: H) -> Client<H, B>
    where
        H: HttpConnect,
        B: Body + Send,
        B::Data: Send,
