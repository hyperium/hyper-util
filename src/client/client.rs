        ...
        match req.version() {
            ...
            Version::HTTP_10 => {
                if is_http_connect {
                    ...
                    return ResponseFuture::new(future::err(crate::GenericError {}));
                }
            }
            Version::HTTP_2 => (),
    ...
    async fn send_request(
        ...
        if pooled.is_http1() {
            if req.version() == Version::HTTP_2 {
                ...
                return Err(ClientError::Normal(crate::GenericError {}));
            }
    ...
    fn connect_to(
        ...
        hyper_lazy(move || {
            ...
            let connecting = match pool.connecting(&pool_key, ver) {
                Some(lock) => lock,
                None => {
                    let canceled = crate::GenericError {};
                        ...
                        let connecting = if connected.alpn == Alpn::H2 && !is_ver_h2 {
                            match connecting.alpn_h2(&pool) {
                                ...
                                None => {
                                    ...
                                    let canceled =
                                        crate::GenericError {}.with("ALPN upgraded to HTTP/2");
                                    return Either::Right(future::err(canceled));
                                }
                            }
                        } else {
...
impl ResponseFuture {
    ...
    fn error_version(ver: Version) -> Self {
        ...
        ResponseFuture::new(Box::pin(future::err(crate::GenericError {})))
    }
}
...
#[allow(missing_debug_implementations)]
enum ClientError<B> {
    Normal(crate::GenericError),
    Canceled {
        connection_reused: bool,
        req: Request<B>,
        reason: crate::GenericError,
    },
}
...
impl<B> ClientError<B> {
    fn map_with_reused(
        conn_reused: bool,
    ) -> impl Fn((crate::GenericError, Option<Request<B>>)) -> Self {
        move |(err, orig_req)| {
...
enum ClientConnectError {
    Normal(crate::GenericError),
    H2CheckoutIsClosed(crate::GenericError),
}
