...
/// A generic callback to be triggered when a connection is established.
#[derive(Clone)]
pub struct CaptureConnection(std::sync::Arc<dyn Fn(&Connected) + Send + Sync>);

impl CaptureConnection {
    /// Create a new `OnConnection` callback.
    pub fn new<F>(f: F) -> Self
    where
        F: Fn(&Connected) + Send + Sync + 'static,
    {
        Self(std::sync::Arc::new(f))
    }

    /// Execute the callback with the connection information.
    pub fn call(&self, c: &Connected) {
        (self.0)(c)
    }
}

impl std::fmt::Debug for CaptureConnection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OnConnection").finish()
    }
}
