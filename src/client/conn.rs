...
/// See [`client::conn`](hyper::client::conn) for more.
pub async fn handshake<T>(
    io: T,
) -> hyper::Result<(SendRequest<hyper::Body>, Connection<T, hyper::Body>)>
where
...
enum ResponseFutureState {
    ...
    Error(Option<crate::GenericError>),
}
...
impl<B> SendRequest<B> {
    ...
    pub fn poll_ready(&mut self, cx: &mut task::Context<'_>) -> Poll<hyper::Result<()>> {
        self.dispatch.poll_ready(cx)
    }

    pub(super) async fn when_ready(self) -> hyper::Result<Self> {
    ...
    B: HttpBody + 'static,
{
    ...
    pub(super) fn send_request_retryable(
        &mut self,
        req: Request<B>,
    ) -> impl Future<Output = Result<Response<Body>, (crate::GenericError, Option<Request<B>>)>> + Unpin
    where
        B: Send,
    {
    ...
    B: HttpBody + 'static,
{
    pub(super) fn send_request_retryable(
        &mut self,
        req: Request<B>,
    ) -> impl Future<Output = Result<Response<Body>, (crate::GenericError, Option<Request<B>>)>>
    where
        B: Send,
    {
        match self.dispatch.try_send(req) {
    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    pub fn poll_without_shutdown(&mut self, cx: &mut task::Context<'_>) -> Poll<hyper::Result<()>> {
    ...
    pub fn without_shutdown(self) -> impl Future<Output = hyper::Result<Parts<T>>> {
        let mut conn = Some(self);
        future::poll_fn(move |cx| -> Poll<hyper::Result<Parts<T>>> {
...
impl Builder {
    ...
    pub fn handshake<T, B>(
        &self,
        io: T,
    ) -> impl Future<Output = hyper::Result<(SendRequest<B>, Connection<T, B>)>>
    where
        T: AsyncRead + AsyncWrite + Unpin + Send + 'static,
        B: HttpBody + 'static,
        B::Data: Send,
...
where
    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    type Output = hyper::Result<proto::Dispatched>;
